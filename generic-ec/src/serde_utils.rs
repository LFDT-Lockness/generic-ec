...
pub struct Bytes<E = ()>(PhantomData<E>);
...
impl<'de, E, T> DeserializeAs<'de, T> for Bytes<E>
where
    ...
    T: AsMut<[u8]> + Default,
{
    ...
    where
        D: Deserializer<'de>,
    {
        deserialize_bytes::<'de, D, E, T>(deserializer)
    }
}
...
pub fn serialize_bytes<S: Serializer>(serializer: S, bytes: &[u8]) -> Result<S::Ok, S::Error> {
    #[cfg(feature = "std")]
...
pub fn deserialize_bytes<'de, D: Deserializer<'de>, E: Expectation, T: AsMut<[u8]> + Default>(
    deserializer: D,
) -> Result<T, D::Error> {
